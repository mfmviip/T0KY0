...
async def startgmute(event):
    ...
    try:
        ...
        if reason:
            await edit_or_reply(
                event,
                f"✘ ∫ المستخدم تم كتمه من هنا",
            )
        else:
            await edit_or_reply(
                event,
                f"{_format.mentionuser(user.first_name ,user.id)} ✘ ∫  تم كتمه من هنا",
            )
    if BOTLOG:
        reply = await event.get_reply_message()
        if reason:
...
async def endgmute(event):
    ...
    try:
        ...
        if reason:
            await edit_or_reply(
                event,
                f"✘ ∫ المستخدم تم الغاء كتمه من هنا",
            )
        else:
            await edit_or_reply(
                event,
                f"{_format.mentionuser(user.first_name ,user.id)} ✘ ∫  تم الغاء كتمه من هنا",
            )
    if BOTLOG:
        if reason:
            await event.client.send_message(
